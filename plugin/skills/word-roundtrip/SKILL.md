...
| `NOT merged` | refused, with the reason under it: a number or citation changed (`'3.84' comes from results.ror.point`), the paragraph was split or has new text beside it, a heading was joined into it, text was typed where it renders nothing, the edited text carries markup Word's text cannot bring back (named: a footnote, an HTML comment, a link, an equation, raw TeX…), merged it would not read as the text that came back, the text between two numbers or citations was deleted so they would touch, Word draws something in it that has no text the source can hold (named: `Wingdings character F04A`, often an AutoCorrect symbol for `:)` or `-->`, or a piece of a tall bracket; retype it as the character itself, or make the edit in the `.md`), it holds a private-use character (named: `private-use character F06D`, a symbol or icon font's own code, which the build would draw in the body font; retype the character itself), part of it is in the Symbol font because a style, the defaults or the theme sets it (read, but not taken as exact; set the font on the text itself), or it could not be lined up with its source. The whole paragraph is refused, including any rewording in it |

...
| `moved into a different section or file` | a move past a heading, table, figure, list, quotation or other block without an identifier, past a paragraph import holds in place (one with a line such as `\end{table}` directly under it, or one directly above an equation that stands on its own), or into another file. A held paragraph dragged past two paragraphs or more is named here itself; dragged past one, that one is named. Not applied; move it in the `.md` yourself |

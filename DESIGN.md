...
- **A link or footnote definition carries no identifier.** Pandoc reads `[reg]: https://...`
  ...
  - *Only the plainest shapes count.* A link is a label, one token for its address and
    ...
    which `import` does not compare. A link written straight above a paragraph or a
    ...
    title on the next line, a nested bracket in its label, a footnote whose text wraps or
    that is written straight above prose, or a link over a line opening with a quote, a

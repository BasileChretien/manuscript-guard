...
| `paragraph(s) without an identifier … came back different` | a heading, list item, quotation, caption, new paragraph, or paragraph tied to markup in or around it (such as display maths, a comment closing past it, or a fence, `</div>` or definition directly under it) was edited (`-` the old text, `+` the new), deleted or added. Not applied; make the edit in the `.md`. A paragraph moved past one of these may not be reported as moved, so compare the documents as text (below) |

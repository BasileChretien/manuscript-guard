...
- Send `build/manuscript.docx`, and `build/supplementary.docx` if the supplement needs
  ...
  comes back as new text without an identifier, listed but not applied, and several are
  refused. Move text between them in the .md. In a project with a supplement, a document
  that has lost every identifier is refused, and so is a supplement of only headings,
  tables and figures, which holds nothing to compare.
...
| `moved into a different section or file` | a move past a heading, table, figure, list, quotation or other block without an identifier, past an HTML comment or `\newpage`, which Word does not show, past a paragraph import holds in place (one Word shows as an empty line, one with a line such as `\end{table}` directly under it, one with a `<!--` that never closes, or one Word shows as more than one paragraph), or into another file. A held paragraph dragged past two paragraphs or more is named here itself; dragged past one, that one is named. Not applied; move it in the `.md` yourself |
| `came back somewhere else` | a heading, table, figure, caption or equation was dragged to another place in Word. When the text without an identifier all came back, only reordered, a heading, list item or caption out of place is listed in the `different order` row instead. Not applied; move the heading, the table's or figure's placeholder, or the equation in the `.md` yourself; a caption goes with its table or figure |
...
| `paragraph(s) without an identifier … came back different` | a heading, list item, quotation, caption, new paragraph, or paragraph with display maths, with a comment that closes past it, or with a fence, `</div>` or definition directly under it was edited (`-` the old text, `+` the new), deleted or added. Not applied; make the edit in the `.md`. A paragraph moved past one of these may not be reported as moved, so compare the documents as text (below) |
...
| `N of M paragraphs … carry no identifier` | headings, table cells, captions, list items, block quotes, paragraphs with display maths, with a comment that closes past them, or with a fence, `</div>` or definition directly under them, and new paragraphs. **None of these was compared**; those outside tables that changed are listed by the rows above, and an edit inside a table is not reported at all |

Anything refused, joined, deleted or moved between sections or files, any heading, table,
figure or equation that was moved or could not be found, and any paragraph without an
identifier that came back different or in a different order, makes the command exit 1, with
or without `--apply`; the safe changes are still applied.
...
- A move beside an HTML comment with a blank line in it no longer writes the moved paragraph
  inside the comment. A paragraph with a fence, `</div>`, `\end`, a definition or a
  heading's underline written directly under it is neither moved nor reworded: its edit is
  listed or refused.

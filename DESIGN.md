...
`$\text{``crude''}$` did the same. So the source is also read a second way, which first sets
aside what pandoc reads before code spans: raw TeX with its arguments, maths, a link's

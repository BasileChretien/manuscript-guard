...
- **A tracked change is accepted, not shown.** The import reads the document as if every
  ...
  the new one, so it is reported rather than applied. A table is the exception: one
  deleted as a tracked change is still read as a table. Import never deletes a table from
  the source, so there it still stands between the paragraphs either side of it, and it
  is held fixed with the others when judging where a paragraph moved. Read as gone, the
  tables returned no longer lined up with those sent, none was held fixed, and a paragraph
  dragged below the next table was applied above it (2026-09-25).
- **A join into or past a table is neither applied nor reported.** A paragraph whose mark
  was deleted just before a table runs on in Word, into the table's first cell, or past a
  table deleted whole into the paragraph after it. Import folds a paragraph into the next
  only when no table stands between them, so it reads the paragraph as it was, unchanged,
  and the co-author's join is lost without a word. The table is still there to be read,
  since a deleted table is kept (above); refusing such a paragraph as a join would be the
  way to report it.

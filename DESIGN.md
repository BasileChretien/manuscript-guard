...
- **A link or footnote definition carries no identifier.** Pandoc reads `[reg]: https://...`
  ...
  - *A definition between two paragraphs is no section boundary.* It renders nothing in the
    ...
    marks by, so a definition `tag` marks - under a line pandoc does not take for blank, in
    a shape it could read otherwise, or a note that would run on into what is below - is a
    paragraph, not something between two. A move across a definition can also carry such a
    paragraph to where it becomes one (above).

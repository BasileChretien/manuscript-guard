...
- **Merging a reworded paragraph drops inline math.** A paragraph is read from its `w:t`
  runs, and Word keeps math in `m:t`: `The hazard was $\lambda$ per year.` reaches the
  merge as "The hazard was per year.", and a co-author's rewording of it comes back as a
  source paragraph with the symbol gone. Nothing is reported. A footnote, a link's address,
  display math and an HTML comment are refused rather than merged for the same reason (see
  "The round trip carries prose"); inline `$...$` is not yet among them.
